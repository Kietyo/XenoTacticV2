...
import pathing.HorizontalDirection
import pathing.VerticalDirection
import kotlin.math.*
...
inline fun <T> measureTime(
    ...
    block: () -> T
        ...
        Pair<Long, T> {
    val start = System_nanoTime()
    val ret = block()
    val duration = System_nanoTime() - start
    if (printMessage) {
        val asMillis = duration / 1e6
        if (message == null) {
            println("Took $duration nanos ($asMillis millis)")
        } else {
            println("$message: Took $duration nanos ($asMillis millis)")
        }
    }
    return Pair(duration, ret)
}
